...
class Show (db.Model):
    ...
    def __repr__(self):
      return f'<Show Time: {self.start_time}, Venue Id: {self.venue_id}, Artist: {self.artist_id}>'
...
def venues():
  ...
  for area in all_areas:
    venues_in_same_area = Venue.query.filter_by(state=area.state).filter_by(city=area.city).all()
    venue_data = []
    for venue in venues_in_same_area:
      venue_data.append({
        "id": venue.id,
        "name": venue.name, 
      })
...
def search_venues():
  ...
  founded_venues= Venue.query.filter(Venue.name.ilike(f'%{search_term}%')).all()
  # TODO: implement search on artists with partial string search. Ensure it is case-insensitive.(done)
  # seach for Hop should return "The Musical Hop".
  # search for "Music" should return "The Musical Hop" and "Park Square Live Music & Coffee"
  data=[]
  if founded_venues : 
    for venue in founded_venues:
      data.append({
        "id": venue.id,
        "name": venue.name,
      })
    ...
    return redirect(not_found_error) 
...
def show_venue(venue_id):
  ...
  all_past_shows= Show.query.join(Artist).filter(Show.venue_id==venue.id).filter(Show.start_time<datetime.now()).all()
  all_upcoming_shows= Show.query.join(Artist).filter(Show.venue_id==venue.id).filter(Show.start_time>datetime.now()).all()
  past_shows=[]
  upcoming_shows=[]
...
def delete_venue(venue_id):
  ...
  try:
    ...
    db.session.close()
  if error: 
    flash(f'An error occurred. Venue {venue_id} cannot be deleted.')
  if not error: 
...
def search_artists():
  ...
  search_term=request.form.get('search_term', '')
  founded_artists= Artist.query.filter(Artist.name.ilike(f'%{search_term}%')).all()
  if founded_artists :
    data=[]
...
def show_artist(artist_id):
  ...
  all_past_shows= Show.query.join(Venue).filter(Show.artist_id==artist.id).filter(Show.start_time<datetime.now()).all()
  all_upcoming_shows= Show.query.join(Venue).filter(Show.artist_id==artist.id).filter(Show.start_time>datetime.now()).all()
  past_shows=[]
  upcoming_shows=[]
